...
class AdminController extends \admin\components\Controller
{
    ...
    public function actionLeaderList()
    {
        ...
        $html = $query->getTable([
            ...
            'retail.total_fee' => ['type' => 'text'],
            'leader.point' => ['header' => '返点%'],
            'state' => ['search' => 'select'],
            ['header' => '操作', 'width' => '80px', 'value' => function ($row) {
                return Hui::primaryBtn('修改返点', ['editPoint', 'id' => $row->id], ['class' => 'editBtn']) . Hui::primaryBtn('修改密码', ['editAdminPass', 'id' => $row->id], ['class' => 'editBtnPassword']);
            }]
        ], [
    ...
    public function actionSaveLeader($id = null)
    {
        ...
        if ($user->load() && $adminLeader->load()) {
            ...
            if ($user->saveAdmin()) {
                ...
                $retail->tel = $adminLeader->mobile;
                $retail->realname = $retail->company_name = $user->realname;
                $retail->insert(false);
                $user->power = AdminUser::POWER_LEADER;
                $user->update();
