...
class SiteController extends \frontend\components\Controller
{
    public function beforeAction($action)
    {
        if (!parent::beforeAction($action)) {
            ...
            $actions = ['ajax-update-status', 'wxtoken', 'wxcode', 'test', 'rule', 'captcha','notify', 'hx-weixin', 'zynotify', 'update-user', 'update', 'tynotify', 'verify-code', 'open-notify', 'bxnotify', 'nqdelete', 'zbnotify', 'new-bxnotify', 'hm-alipaynotify'];
    ...
    {
        $mobile = post('mobile');
        $model = new User(['scenario' => 'verify']);
    ...
    **/
    ...
    // 钱通
    public function actionYzchangeNotify() //交易所微信支付
    {
        ...
        if(verity(MD5($resp_xml,true),$resp_sign)){//验签
            $data = fromXml($resp_xml);
                $userCharge = UserCharge::find()->where('trade_no = :trade_no', [':trade_no' => $data['@attributes']['merchantOrderId']])->one();
                //有这笔订单
                if (!empty($userCharge)) {
                    $total_fee = $userCharge->amount;
                    if ($userCharge->charge_state == UserCharge::CHARGE_STATE_WAIT) {
                        $user = User::findOne($userCharge->user_id);
    ...
    //商品支付回调
       public function actionShopnotify() //中云支付回调
    {
        $data = $_GET;
        if (isset($data['returncode']) && $data['returncode'] == '00') {
            $return = [
            ...
            // var_dump($newSign);
            if ($data['sign'] == $newSign) {
                $userCharge = ShopOrder::find()->where('trade_no = :trade_no', [':trade_no' => $data['orderid']])->one();
                //有这笔订单
                if (!empty($userCharge)) {
                    $tradeAmount = $data['amount'];
                    if ($userCharge->charge_state == UserCharge::CHARGE_STATE_WAIT) {
    ...
    // 亿支付
    public function actionZbnotify() 
    {
        $data = $_GET;
        if (isset($data['returncode']) && $data['returncode'] == '00') {
            $return = [
            ...
            $string .= "key=" . ZBPAY_KEY;
            $newSign = strtoupper(md5($string));
            if ($data['sign'] == $newSign) {
                $userCharge = UserCharge::find()->where('trade_no = :trade_no', [':trade_no' => $data['orderid']])->one();
                //有这笔订单
                if (!empty($userCharge)) {
                    $tradeAmount = $data['amount'] * 98.5 / 100;
                    if ($userCharge->charge_state == UserCharge::CHARGE_STATE_WAIT) {
                        $user = User::findOne($userCharge->user_id);
                        $user->account += $tradeAmount;
                        if ($user->save()) {
                            $userCharge->charge_state = UserCharge::CHARGE_STATE_PASS;
                            $userCharge->amount = $tradeAmount;
                        }
                    }
                    $userCharge->update();
                }
    ...
    public function actionOpenNotify() //快捷微信支付宝支付
    {
        header('Content-Type:application/json;charset=utf-8');
        $data = file_get_contents('php://input');
        // l($data);
        // $data = '{"CommandID":2324,"SeqID":"1","NodeType":"3","NodeID":"openplat","Version":"1.0.0","TokenID":"","RetCode":0,"ErrorMsg":"","AGENTNO":"","MERNO":"132440359699067","TERMNO":"90780001","Body":{"AMT":1,"ORDERNO":"100000009538","TRDTIME":"2017-06-13 14:33:03","PAYTIME":"2017-06-13 14:33:16","STATUS":"1","MERORDERID":"100001201706138939","TRADETYPE":"1"},"Sign":"B77750E8B9572086C7A5C7C84F0C1C7F"}';
        $object = json_decode($data);

        if (!$object) {
            $object = json_decode(urldecode($data));
        }
        $array = json_decode(json_encode($object),true);

        if ($object->Body->STATUS == 1) {
            require Yii::getAlias('@vendor/pay/base.class.php');
            $base = new \base();
            $result = strtoupper($base->ckSign($array['Body']));
 
            if($result == $array['Sign']) {
                    $userCharge = UserCharge::find()->where('trade_no = :trade_no', [':trade_no' => $array['Body']['MERORDERID']])->one();
                    //有这笔订单
                    if (!empty($userCharge)) {
                        if ($userCharge->charge_state == UserCharge::CHARGE_STATE_WAIT) {
                            $user = User::findOne($userCharge->user_id);
                            $amount = $array['Body']['AMT'] / 100;
                            $user->account += $amount * 100;
                            if ($user->save()) {
                                $userCharge->charge_state = UserCharge::CHARGE_STATE_PASS;
                            }
                        }
        ...
        if (isset($data['sign'])) {
            ...
            if ($sign == $newSign) {
                ...
                if (!empty($userCharge)) {
                    ...
                    $total_fee = $userCharge->amount * 98.5 / 100;
                    if ($userCharge->charge_state == UserCharge::CHARGE_STATE_WAIT) {
                        $user = User::findOne($userCharge->user_id);
                        $user->account += $total_fee;
                        if ($user->save()) {
    ...
    public function actionUpdate()
    {
        $ids = self::db('SELECT o.id, a.price FROM `order` o INNER JOIN product p on p.id = o.product_id INNER JOIN data_all a on a.name = p.table_name where 
            order_state = ' . Order::ORDER_POSITION)->queryAll();
    ...
    public function actionNqdelete()
    {
        ...
        self::db('DELETE FROM data_yushi WHERE time < "'.$time.'"')->query();
