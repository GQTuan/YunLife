...
class RetailController extends \admin\components\Controller
{
    ...
    {
        $query = (new Retail)->search()->joinWith('adminUser')->retail()->andWhere(['adminUser.power' => AdminUser::POWER_MANAGER]);
        ...
        $html = $query->getTable([
            ...
            ['header' => '操作', 'width' => '80px', 'value' => function ($row) {
                return Hui::primaryBtn('修改返点', ['editPoint', 'id' => $row->admin_id], ['class' => 'editBtn']) . Hui::primaryBtn('修改密码', ['editAdminPass', 'id' => $row->admin_id], ['class' => 'editBtnPassword']);
            }],
        ], [
